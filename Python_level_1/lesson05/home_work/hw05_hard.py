...
def cp():
    ...
    copy_of_file_path = os.path.join(os.getcwd(), 'copy_' + dir_name)
    try:
        copyfile(file_path, copy_of_file_path)
        print('копия файла {} создана'.format(dir_name))
    except IOError:
        print('директория не доступна для записи')
...
def rm():
    if not dir_name:
        ...
        return
    file_path = os.path.join(os.getcwd(), dir_name)
    if input("Подтвердите удаление файла {}. (y/n): ".format(dir_name)).upper() != "Y":
        return
    try:
        os.remove(file_path)
        print('файла {} удален'.format(dir_name))
    except FileNotFoundError:
        print('файла {} не существует'.format(dir_name))
    except PermissionError:
        print('вы пытаетесь удалить ПАПКУ {}'.format(dir_name))
...
def cd():
    ...
    if os.path.isabs(dir_name):
        try:
            os.chdir(dir_name)
            print('текущая директория {}'.format(dir_name))
        except FileNotFoundError:
